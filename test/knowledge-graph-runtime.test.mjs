...
test("settles independently from contextual animation eligibility", () => {
  const base = {
    connected: true,
    hidden: false,
    reducedMotion: false,
...
test("fit bounds follow the settled nodes instead of the server envelope", () => {
  assert.deepEqual(
    measureGraphContentBounds([
...
test("client wires first settle, restoration, and cancelled-drag reheating", async () => {
  ...
  assert.match(
    source,
    /if \(contextualRelationships\) \{[\s\S]*?warmContextualLayout\(\);[\s\S]*?\} else \{[\s\S]*?startSimulation\(\);[\s\S]*?\}[\s\S]*?updateEdges\(\);[\s\S]*?syncContextualSimulation\(\);/,
  );
  assert.match(source, /window\.addEventListener\("pageshow"/);
  assert.match(
    source,
    /else if \(finishedDrag\.moved\) \{[\s\S]*?startSimulation\(finishedDrag\.nodeId\);/,
  );
});
